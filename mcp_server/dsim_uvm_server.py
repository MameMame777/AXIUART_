...
"""
    ...
    async def _check_dsim_environment(self) -> CallToolResult:
        ...
        status_lines = ["🔍 DSIM Environment Status", "="*50]
        
        # Check DSIM_HOME
        if self.dsim_home and Path(self.dsim_home).exists():
            status_lines.append(f"✅ DSIM_HOME: {self.dsim_home}")
        else:
            status_lines.append(f"❌ DSIM_HOME: Not set or invalid ({self.dsim_home})")
            
        # Check DSIM executable
        if self.dsim_home:
            dsim_exe = Path(self.dsim_home) / "bin" / "dsim.exe"
            if dsim_exe.exists():
                status_lines.append(f"✅ DSIM Executable: {dsim_exe}")
            else:
                status_lines.append(f"❌ DSIM Executable: Not found at {dsim_exe}")
        ...
        if uvm_dir.exists():
            status_lines.append(f"✅ UVM Directory: {uvm_dir}")
        else:
            status_lines.append(f"❌ UVM Directory: Not found at {uvm_dir}")
            
        if config_file.exists():
            status_lines.append(f"✅ Config File: {config_file}")
        else:
            status_lines.append(f"❌ Config File: Not found at {config_file}")
        
        # Check license
        license_env = os.environ.get('DSIM_LICENSE')
        if license_env:
            status_lines.append(f"✅ DSIM License: {license_env}")
        else:
            ...
            if default_license and default_license.exists():
                status_lines.append(f"✅ DSIM License: {default_license} (default)")
            else:
                status_lines.append("⚠️  DSIM License: Not explicitly set (may use default)")
